...
export class InfoJobsRepo implements JobRepository {
  public readonly source = "InfoJobs";
  ...
  async listAll(): Promise<Job[]> {
    try {
      ...
      console.error("Erro ao buscar vagas do InfoJobs:", error);
      return [];
    }
  }
  ...
  async getById(id: string): Promise<Job | null> {
    try {
      ...
      console.error("Erro ao buscar vaga específica do InfoJobs:", error);
      return null;
    }
  }
  ...
  private parseJobs(html: string): Job[] {
    const jobs: Job[] = [];
    const jobRegex =
      /<div[^>]*class="[^"]*job-item[^"]*"[^>]*>.*?<a[^>]*href="([^"]*)"[^>]*>.*?<h2[^>]*>([^<]*)<\/h2>.*?<span[^>]*class="[^"]*company[^"]*"[^>]*>([^<]*)<\/span>.*?<span[^>]*class="[^"]*location[^"]*"[^>]*>([^<]*)<\/span>/gs;
