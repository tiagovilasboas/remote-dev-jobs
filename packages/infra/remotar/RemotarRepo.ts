...
export class RemotarRepo implements JobRepository {
  public readonly source = "Remotar";
  private readonly baseUrl = "https://remotar.com.br";

  async listAll(): Promise<Job[]> {
    try {
      const response = await fetch(`${this.baseUrl}/vagas?tag=frontend`);
      ...
      console.error("Erro ao buscar vagas do Remotar:", error);
      return [];
    }
  }
  ...
  async getById(id: string): Promise<Job | null> {
    try {
      ...
      console.error("Erro ao buscar vaga específica do Remotar:", error);
      return null;
    }
  }
