...
export class HipstersRepo implements JobRepository {
  public readonly source = "Hipsters.jobs";
  private readonly baseUrl = "https://hipsters.jobs";

  async listAll(): Promise<Job[]> {
    try {
      const response = await fetch(`${this.baseUrl}/vagas?categoria=frontend`);
      ...
      console.error("Erro ao buscar vagas do Hipsters.jobs:", error);
      return [];
    }
  }
  ...
  async getById(id: string): Promise<Job | null> {
    try {
      ...
      console.error("Erro ao buscar vaga específica do Hipsters.jobs:", error);
      return null;
    }
  }
